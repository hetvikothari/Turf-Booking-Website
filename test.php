...
<div id="booking" class="container-fluid text-center">
  ...
  <p align="center">SELECT SLOT TIMING TO BOOK</p>
    ...
    <br>
      ...
      <input type="TIME" name="TIME2" value="10:00">
    </div>
    <br>
      <div align="center">
...
{
  echo '<p> BOOKING SUCESSFULL</p>' ;

}
else{
  $sql1="DELETE from user_date_time where user='$name' and date='$date' and TIME1='$stime';";
  mysqli_query($conn,$sql1);

  echo '<p> SOMEONE ELSE HAS ALREADY BOOKED THIS COURT.PLEASE SELECT DIFFERENT TIME SLOT/DIFFERENT DAY/DIFFERENT COURT TO PLAY.</P>';
}

}
else
{
  echo '<p> USER ALREADY HAS A BOOKING ON THIS DATE AT THIS TIME.</p';

}
}
else
{
  echo '<p> Number of players are more than maximum number of players</p>';
}
  ...
  while($row = $result->fetch_assoc())
    {
      ...
      <tr id="number">
        ...
        <td><a href="removerow.php?id=<?php echo $id; ?>"><button id="number" name='delete' class="btnDelete" onclick="delete(<?php echo $id;?>)">DELETE</button></a></td>
      </tr>
  <?php
...
if(isset($_POST['submit']))
{
    ...
    if($conn->query($sql)===TRUE)
{
  echo '<p> MESSAGE SENT SUCESSFULLY !!</p>' ;
